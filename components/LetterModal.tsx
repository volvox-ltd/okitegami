          ...
          <div className="flex justify-center items-center shrink-0 relative h-10 mb-2">
            {!isLocked && !isMyPost && activeLayer === 0 && !hideFavorite && (
              <div className="absolute left-0 z-[45]">
                <button onClick={(e) => { e.stopPropagation(); toggleFavorite(); }} className={`flex items-center gap-1 text-[10px] font-bold py-1.5 px-3 rounded-full border shadow-sm transition-all active:scale-95 ${isFavorited ? 'bg-pink-50 text-pink-500 border-pink-100' : 'bg-gray-100 text-gray-400 border-gray-200'}`}>{isFavorited ? '♥' : '♡'}</button>
              </div>
            )}
