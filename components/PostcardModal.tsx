...
export default function PostcardModal({ letter, currentUser, onClose, onDeleted, onRead }: Props) {
  ...
  useEffect(() => {
    ...
    const initModal = async () => {
      ...
      // ★ 運営（is_official）でない場合のみニックネームを取得
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className={`relative w-full max-w-[380px] aspect-[1/1.48] transition-all duration-700 font-sans preserve-3d ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`} style={{ perspective: '1500px' }}>
        ...
        >
          {!isLocked && (
            <div className="absolute pointer-events-none z-30" style={{ left: 'calc(var(--spacing) * 3)', top: 'calc(var(--spacing) * 3)', width: '72px', height: '90px' }}>
               <div className="relative w-full h-full">
                 {/* ★ 修正：運営なら Postcard_Admin.png、ユーザーなら PostcardStamp.png を表示 */}
                 <Image 
                   src={letter.is_official ? "/Postcard_Admin.png" : "/PostcardStamp.png"} 
                   fill 
                   className="object-contain" 
                   alt="stamp" 
                   priority 
                 />
                 <div className="absolute left-6 top-10 w-16 h-16 border-2 border-black/20 rounded-full flex items-center justify-center flex-col rotate-12 mix-blend-multiply opacity-30">
                    ...
                    <div className="w-full border-t border-black/20 my-0.5" />
                    <span className="text-[6px] text-black/20 font-bold font-sans">{hh}:{min}</span>
                 </div>
               </div>
            </div>
          )}
            ...
            {isLocked ? (
              ...
              <>
                ...
                <div className="flex-1 h-full flex flex-col justify-start pt-12 pb-4">
                   <div 
                     className="text-sm md:text-base leading-[2.2] font-serif tracking-[0.15em] [writing-mode:vertical-rl] whitespace-pre-wrap text-[#5d4037] h-full overflow-hidden modal-html-content"
                ...
                <div className="w-10 h-full flex flex-col justify-end items-center pb-4 shrink-0">
                   <span className="text-[10px] text-[#8a776a] font-serif [writing-mode:vertical-rl] tracking-widest opacity-80 whitespace-nowrap">
                     {/* ★ 修正：運営なら「木林文庫 より」、ユーザーなら「ニックネーム より」を表示 */}
        ...
        {/* --- 裏面 --- */}
        <div className="absolute inset-0 w-full h-full backface-hidden [transform:rotateY(180deg)] bg-black shadow-2xl flex flex-col rounded-sm overflow-hidden cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
          {letter.image_url ? (
            <>
              ...
              <div className="absolute bottom-6 inset-x-6 flex flex-col items-center justify-center z-20">
                <p className="text-white text-[11px] font-bold tracking-[0.5em] ml-[0.5em] mb-3 uppercase drop-shadow-md">{letter.spot_name}</p>
                <span className="text-white/90 text-[10px] font-bold tracking-widest drop-shadow-md">タップで裏返す</span>
              </div>
            </>
          ) : (
